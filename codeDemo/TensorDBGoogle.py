...
from tensor_lab.gc_impl import gc_index_db

gio= gc_file_io.GfileIO()
gdb= gc_index_db.GoogleDocDB()
...
class TensorDB(object):
    ...
    def __SaveParameters(self,params):

        s=self.__serialization(params)

        return gio.UploadContent('parametersfs',s)
	


    def __LoadParameters(self,fname):

        s=gio.DownLoadConent('parametersfs',fname)

        return self.__deserialization(s)

    def __DeleParams(self,fname):
        gio.DeleteFile('parametersfs',fname)
    ...
    def save_params(self, params=[], args={}):#, file_name='parameters'):
        ...
        """
        s = time.time()
        f_id = self.__SaveParameters(params)
        tm=datetime.utcnow()
        args.update({'f_id': f_id, 'time':tm })
        args.update({'id':self.studyID+":"+str(args["epoch"])+':'+tm.isoformat()})
        print args
        gdb.add_new_doc('params',args)
    ...
    @AutoFill
    def find_one_params(self, args={},sort=[]):
        ...
        s = time.time()
        print args
        d = gdb.query('params',filters=args,sorts=sort)
        ...
        if d is not None:
            ...
            return False, False
        try:
            params = self.__LoadParameters(f_id)
    ...
    def find_all_params(self, args={}):
        ...
        s = time.time()
        pc = gdb.query('params',args)

        if pc is not None:
            f_id_list = [ d['fid'] for d in pc]
            f_id_list=set(f_id_list)
            params = []
            for f_id in f_id_list: # you may have multiple Buckets files

                params.append(self.__LoadParameters(f_id))
        else:
    ...
    def train_log(self, args={}):
        ...
        args.update({'id':self.studyID+":"+str(args['batch'])+":"+args["time"].isoformat()})
        _result = gdb.add_new_doc('train_log',args)
        _log = self._print_dict(args)
    ...
    def del_train_log(self, args={}):
        ...
        gdb.delete_docs('train_log',args)
    ...
    def valid_log(self, args={}):
        ...
        """
        args.update({'id': self.studyID + ":" + str(args['epoch']) + ":" + args["time"].isoformat()})
        _result = gdb.add_new_doc('val_log',args)
...
if __name__=="__main__":
    db=TensorDB()
    db.save_params("133456",args={'acc':0.9,"epoch":20})
    db.find_one_params()
    db.train_log({"great":"hello",'batch':1,"time":datetime.utcnow()})
    db.valid_log({"great": "hello", 'epoch': 1, "time": datetime.utcnow()})
    print "done"
